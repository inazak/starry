...
func TestParser(t *testing.T) {
  ...
  for i, p := range ps {
    ...
    if list[0].Decode() != p.Inst {
      t.Errorf("No.%d expected=%v got=%v", i, p.Inst, list[0].Decode())
    }
  }
}
...
func TestStackOp(t *testing.T) {
  ...
  if out.String() != expected {
    t.Fatalf("Stdout expected=`%s` got=`%s`", expected, out.String())
  }
}
...
func TestNumberInputOp(t *testing.T) {
  ...
  if out.String() != testdata {
    t.Fatalf("Stdout expected=`%s` got=`%s`", testdata, out.String())
  }
}
...
`
  ...
  if buf.String() != expected {
    t.Fatalf("Stdout expected=`%s` got=`%s`", expected, buf.String())
  }
}
